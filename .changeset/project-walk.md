...
Add a `walk` subcommand: walk a set of cards one at a time for hands-on triage/grooming. Each card gets a concise block (title, status, key signals, short body preview) and a compact decision menu — status move / edit / comment / add-to-milestone / decompose / delete / dig / skip / open / more — applied per card as you go. The scope is described in the user's own words (a milestone, "the Todo column", "the `stale` cards", or nothing → everything not-Done); the agent resolves it via the board's existing server-side filtering rather than any user-typed query flag.
