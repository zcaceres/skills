...
- `/stacked-pr checkpoint [description]` — ports the workflow from the
  former `checkpoint` skill (removed in the same release; see the
  `stacked-pr-remove-checkpoint` changeset). Cuts the uncommitted diff
...
`stacked-pr-remove-checkpoint` changeset). The other sibling skills
