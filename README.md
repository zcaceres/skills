...
| `project` | **Slash command.** Manage a repo's project-tracker kanban board with one skill over a pluggable backend (GitHub Projects or Linear via the official Linear MCP) — `setup`, `next`, `new-task`, `update`, `review`, `decompose`, `delete` subcommands. |
