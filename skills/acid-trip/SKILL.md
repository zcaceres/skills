...
5. **Self-critique pass. Mandatory.** Before stamping provenance:
   ...
   **(a) Audit.** Capture the current state — `get_screenshot` the artboard (Paper) or read the rendered file (HTML/React). Walk through the **Hard blacklist** and the **Forbidden micro-anatomy** lists item by item and ask, for each: *does the current build contain this?* Be specific. Name the section, quote the element. Examples of good audit notes:
   ...
   >
   > "Yes — the subtitle under the headline is italic ('*field notes from the northern shipyards*'), and three body paragraphs each italicize one adjective for flavor. That's gratuitous italic emphasis — decoration, not typesetting."
   >
   ...
   **(b) Rebuild the violations.** For each named violation, *restructure* — don't tweak. A few sample moves:
   ...
   - Gratuitous italic emphasis → un-italicize. Set the word in roman; if it genuinely needs emphasis, reach for the lineage's own device (weight, caps, size, color, letterspacing) instead of a reflexive slant. Reserve true italic for its real jobs — titles of works, foreign terms, citation.
...
- ❌ **Gratuitous italic emphasis.** Single words and stray phrases set in italic for "emphasis" or borrowed elegance — *definitive*, *the* record, an italic adjective per paragraph, an italic tagline floating under the headline, italic pull-quotes and captions by default. Italics are a tool with specific jobs (titles of works, foreign terms, true contrastive stress, citation), not ambient decoration; AI reaches for them to fake "voice." In the output artifact, italicize only where a real typesetter of the rolled document_type would — usually almost nowhere. If a word needs emphasis, use the lineage's own device (weight, caps, size, color, letterspacing), not a reflexive slant.
