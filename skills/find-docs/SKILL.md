...
Run the latest CLI on demand. `--yes` prevents an interactive package-install
prompt from blocking the agent:

```bash
npx --yes ctx7@latest <command>
```

Users who prefer a global installation can install the same version separately,
but all commands below use the installation-free form:
...
You MUST call `npx --yes ctx7@latest library` first to obtain a valid library ID UNLESS the user explicitly provides a library ID in the format `/org/project` or `/org/project/version`.
...
npx --yes ctx7@latest library nextjs "How to set up app router with middleware"
...
The available versions are listed in the `npx --yes ctx7@latest library` output. Use the closest match to what the user specified.
