# `/gh-project batch` — Create, Update, or Delete Many Cards at Once

You are applying the same kind of operation to **several** cards on the GitHub
Projects kanban board in one invocation: creating a batch of new cards, updating
a set of existing cards, or removing a set of cards.

**This subcommand is an envelope, not a reimplementation.** The per-card `gh`
recipes already live in the single-card references — `batch` reuses them:
...
failed item and reports a per-item tally.** When you need the exact flags for a
given card type (issue vs draft, milestone/label inheritance, sub-issue wiring),
open the matching single-card reference rather than guessing here.
...
- "/gh-project batch create|update|delete [args]"
...
Same gate as every other subcommand. If `.github/gh-project.json` or the helper
is missing, **stop and route to `/gh-project setup`** — never guess IDs.
...
TITLE_FIELD=$(jq -r .title .github/gh-project.json)
```

The helper's `find-many` subcommand must exist for `update`/`delete` (it
resolves every selector against a single board fetch). If the installed helper
predates it (`find-many` errors with `unknown subcommand`), tell the user to
re-run `/gh-project setup` to refresh the script, then fall back to looping
`find` per selector in the meantime.
...
3. **A board query** (`update`/`delete` only) — `batch <mode> --query "<q>"`
   ...
   `--query "label:stale -status:Done"`). Resolve with the helper:
   ```bash
...
- **delete** — **require the user to type `yes`** after seeing the full set and
  the per-type consequences (draft = destroyed, issue = unlinked). Anything
  other than an affirmative cancels the entire batch. See [Mode: delete](#mode-delete).
...
fallback): **[new-task.md](new-task.md)** — read it for the exact `gh issue
create` / `gh project item-create` flags. This mode only adds the batch envelope
and **shared defaults**.
...
```jsonc
{
  ...
  "mode": "issue",                       // optional; default = config defaultMode
  "labels": "bug,p1",                    // optional; issues only
  "milestone": "v0.4",                   // optional; issues only
  "assignees": "@me",                    // optional; issues only
...
For each accepted item, run the create recipe from `new-task.md` (Mode A issue
by default, Mode B draft on request), folding in the shared defaults. Capture
the new issue URL / `PVTI_…` per item. If an item wants a non-`Todo` initial
status, move it with the helper after creation:
...
one-field-per-`item-edit`; append-vs-replace body discipline):
**[update.md](update.md)**. This mode adds selector resolution and the batch
envelope.
...
```jsonc
{
  "selector": "42",                 // PVTI_… | issue# | title substring | (from --query: the .id)
...
Pass all selectors to `find-many` (one board fetch, not one per selector):
...
resolved board rows — skip `find-many` and carry each `.id` directly.
...
For each accepted card, run the update recipe from `update.md`: title/body via
`gh issue edit` (issues) or `gh project item-edit --id` (drafts); status via
`$HELPER set-status "$ITEM_ID" "<status>"`. Remember an issue-backed
`item-edit` is one field per call — loop fields per card. Capture per-card
outcome for the tally.

## Mode: delete

Remove a set of cards from the board. **This is the destructive mode — it keeps
the strongest gate.** Per-item recipe and the full draft-vs-issue consequence
breakdown: **[delete.md](delete.md)**. Batch does not weaken any of delete's
safety rules; it just lets the user confirm the *set* once instead of N times.
...
Same as update — `find-many` (or `--query`), then triage `matchCount`:
...
For issue-backed cards, surface the unlink-vs-close-vs-delete distinction from
`delete.md`. Removing from the board (`gh project item-delete`) leaves the
...
issues" as a second typed confirmation (it's irreversible and admin-gated).

### Apply

For each confirmed card, run the delete recipe from `delete.md`:

```bash
gh project item-delete "$PROJECT_NUMBER" --owner "$PROJECT_OWNER" --id "$ITEM_ID"
```

Continue past a per-item failure; capture outcomes. Note: `gh project
item-delete` (and `gh issue close`/`delete`) are **deliberately not in the
`/gh-project setup` permission allowlist**, so each call may prompt for
approval. That friction is intentional on a bulk-destructive path — don't try to
route around it with a wrapper or `--yes` shortcut.
...
- **Mixed card types in update/delete.** A batch may span issues and drafts.
  That's fine — apply the right per-type recipe per card. Surface the type in the
  preview so the user sees, e.g., which deletes are destructive (draft) vs
  unlinks (issue).
...
- **`find-many` unavailable** (older installed helper). Fall back to looping
  `find` per selector and tell the user to re-run `/gh-project setup` to refresh
  the helper. Don't hand-roll the `item-list` + `jq` resolution inline — that's
  exactly the truncation/ID-mixing trap the helper exists to prevent.
...
- **Reuse, don't reimplement.** The per-card `gh` recipes live in `new-task.md`,
  `update.md`, and `delete.md`. If you find yourself re-typing `gh issue create`
  flags here, stop and read the single-card reference instead — it has the edge
  cases (project-link fallback, milestone inheritance, one-field-per-item-edit)
