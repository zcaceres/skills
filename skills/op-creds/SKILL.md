...
hooks:
  PreToolUse:
    - matcher: "Bash"
      type: command
      command: "~/.claude/skills/op-creds/scripts/run.sh"
---
...
Pairs naturally with the `dont-read-dot-env` skill: `dont-read-dot-env`
prevents the wrong pattern (reading `.env`); `op-creds` enables the right
one.
...
session, the `dont-read-dot-env` skill, and an honest threat model.
...
(`~/.claude/skills/op-creds/`). If you install elsewhere, paste this
    ...
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": [
          { "type": "command", "command": "<path>/op-creds/scripts/run.sh" }
        ]
      }
    ]
  }
...
```sh
ln -s ~/.claude/skills/op-creds/scripts/with-creds ~/.local/bin/with-creds
```
...
2. `run.sh` picks the right bundled binary from `scripts/bin/` for the
   host OS/arch (`op-creds-darwin-arm64`, `op-creds-linux-x64`, or
   `op-creds-windows-x64.exe`).
