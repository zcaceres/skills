...
hook config, and the small prebuilt **binary** it execs has to be
...
- **Codex** → `--agent codex` (wires a `PostToolUse` hook into `~/.codex/hooks.json`)
...
rather than leaning on auto-detection — it can't tell the hosts apart when
multiple host config dirs exist and defaults to Claude Code. If you genuinely
...
`install.sh` needs `jq`, backs up the hook config with a timestamp before
...
repo-local `./.claude`, `./.codex`, or `./.gemini` config instead of user
scope. After a Codex install, use `/hooks` to review and trust the new command
hook; Codex skips untrusted hooks. Re-run the installer if you ever see a
"binary not found" note in your hook logs.
...
- This subcommand reads and writes `git config pr.draft` and
  ...
  (step 5, via `install.sh`, which edits the host's hook config and
