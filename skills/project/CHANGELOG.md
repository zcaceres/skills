...
- b040ce5: Require explicit `a` / `apply` input to accept a `/project walk` suggestion rather than using Enter, which cannot be sent as a standalone chat hotkey.

## 0.2.0

### Minor Changes

- 6cb253f: Add `/project walk` for card-by-card grooming of a milestone, status/column, label, or default not-Done scope. Each card shows an evidence-backed status-or-keep suggestion; Enter accepts it, while status, update, keep, details, delete, and quit remain available. Actions apply as the walk advances and retain the existing update and delete safety gates.
