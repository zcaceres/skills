...
| `walk [scope]` | Walk a set of cards one at a time: a concise block plus a decision menu (status / edit / comment / milestone / decompose / delete / dig / skip) applied per card as you go. Describe the `scope` in words (a milestone, "the Todo column", "the `stale` cards", or nothing → everything not-Done) and the agent resolves it via the board's filtering. Each card carries a light codebase-context signal (was it done elsewhere? did its premise drift?); `dig` escalates one card to deep reasoning. Interactive triage/grooming — an envelope over update / milestone / decompose / delete that keeps every per-card safety rule. |
