...
If the user invoked `/project new-task` with arguments, parse the title from the args; ask for any missing fields. If they invoked it bare, ask for title and body.
...
did. Then read the two github-specific fields this subcommand also uses:

```bash
CFG=.project/config.json
TITLE_FIELD=$(jq -r '.title' "$CFG")         # project title, for `gh issue create --project`
DEFAULT_MODE=$(jq -r '.defaultMode' "$CFG")  # issue | draft
```
...
ITEM_ID=$(.project/scripts/board.sh find "$ISSUE_NUMBER" | jq -r '.id')

.project/scripts/board.sh set-status "$ITEM_ID" "In Progress"
```

If `.project/scripts/board.sh` is missing (setup was run before this script existed), fall back to the raw command — but mention the gap to the user and offer to re-run `/project setup`:

```bash
PROJECT_ID=$(jq -r .projectId .project/config.json)
STATUS_FIELD_ID=$(jq -r .statusField.id .project/config.json)
IN_PROGRESS_ID=$(jq -r '.statusField.options["In Progress"]' .project/config.json)
...
signal. If they invoked `/project next` instead, that subcommand hands off
context for starting work — this one doesn't.

## Edge cases

- **No `.project/config.json`.** Stop and route to `/project setup`. Do not attempt to guess the project number.
...
- Don't silently change `defaultMode`. If the user wants the default flipped, suggest re-running `/project setup`.
