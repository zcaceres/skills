...
User-triggered only. Activate when the user invokes `/quality-socket`. Do not self-activate on related natural-language phrasing ("add socket", "set up socket", a mention of typosquats or risky deps) — surface the slash command to the user instead and let them decide.
...
3. Copy it. Then:
     ...
     gh secret set SOCKET_SECURITY_API_KEY --body '<paste the token>' \
       --repo __REPO__
```
