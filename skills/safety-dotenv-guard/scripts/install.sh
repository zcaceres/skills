#!/usr/bin/env bash
# Wire the safety-dotenv-guard PreToolUse hook (matchers: Read, Bash, Grep, Glob)
# into a Claude Code settings.json so it fires on every matching tool call,
# not just when the skill is loaded into context. Idempotent — re-running is
# a no-op.
#
# Usage:
...
SKILL_NAME="safety-dotenv-guard"
