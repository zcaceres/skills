...
- 5a5a7a5: Block via the PreToolUse `permissionDecision: "deny"` JSON contract on stdout
  ...
  the destructive git command through — so the old exit-2 path was a silent
  no-op on Codex. Adds a "Codex CLI" install section (manual `~/.codex/config.toml`
