...
// arg is still caught. Mirrors the technique used by safety-dotenv-guard.
...
async function main(): Promise<void> {
  try {
    ...
    // safety-dotenv-guard's posture, with louder reporting.
    const msg = err instanceof Error ? `${err.name}: ${err.message}` : String(err);
    console.error(`safety-op-creds: hook error (failing open, NOT screening this call): ${msg}`);
    process.exit(0);
  }
}
