#!/usr/bin/env bash
# Wire the safety-op-creds PreToolUse:Bash hook into a Claude Code
# settings.json so bare `op read` and other secret-printing op subcommands
# are blocked on every Bash call, not just when the skill is loaded into
# context. Idempotent — re-running is a no-op.
#
# Usage:
...
if ! command -v op >/dev/null 2>&1; then
  echo "Note: 1Password CLI ('op') is not installed. The hook still blocks bare"
  echo "'op read' calls, but the with-creds wrapper and \`op run\` workflow won't"
  echo "work until you install it:"
  echo "  macOS: brew install --cask 1password-cli"
  echo "  Cross-platform: https://developer.1password.com/docs/cli/get-started"
  echo
fi
