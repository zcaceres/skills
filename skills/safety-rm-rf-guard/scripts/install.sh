#!/usr/bin/env bash
# Wire the safety-rm-rf-guard PreToolUse:Bash hook into a Claude Code
...
# Idempotency: bail if our hook command is already wired anywhere
# under the right event.
...
if ! command -v trash >/dev/null 2>&1; then
  echo "Note: 'trash' CLI is not installed. The guard still blocks rm/shred/unlink,"
  echo "but the suggested replacement command won't be runnable until you install it:"
  echo "  macOS: brew install trash"
  echo "  Cross-platform: npm install -g trash-cli"
  echo
fi
