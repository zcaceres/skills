...
description: Bundled stacked-PR workflow as one skill optimized specifically for Gemini CLI. Subcommands ship the next slice (checkpoint), update the current branch's PR (update), push the whole stack (submit), visualize it (log), rebase onto trunk (sync), and merge bottom-up (merge). Also ships an AfterTool hook that nudges toward /stacked-pr-gemini checkpoint when the uncommitted diff grows large. Uses git stack when installed, falls back to gh + git. Invoke via /stacked-pr-gemini [subcommand] [args].
...
Run the full stacked-PR workflow as `/stacked-pr-gemini <sub> [args]`. Bundles the
...
An AfterTool hook is also shipped with this skill. It fires after
every `replace`/`write_file` tool call and nudges
toward `/stacked-pr-gemini checkpoint` when the uncommitted diff crosses
...
3. **First token is anything else, OR `$ARGUMENTS` is empty** → default to
   ...
   This means `/stacked-pr-gemini` ≡ `/stacked-pr-gemini checkpoint` (no args), and
   `/stacked-pr-gemini "fix the retry loop"` runs checkpoint with that slice
