# `/stacked-pr-gemini setup` — Choose Normal or Stacked Mode

Show the current `/stacked-pr-gemini` mode and switch between **normal** (default) and
**stacked**. The mode is stored in `git config stacked-pr-gemini.mode` and controls only
what the bare `/stacked-pr-gemini` (default) action does:

- **normal** → `/stacked-pr-gemini` commits your conversation changes, pushes, and opens
  a single PR against the trunk (see [update.md](update.md)).
- **stacked** → `/stacked-pr-gemini` cuts the current diff as the next branch in a stack
...
```bash
echo "local:  $(git config --local stacked-pr-gemini.mode 2>/dev/null || echo '(unset)')"
echo "global: $(git config --global stacked-pr-gemini.mode 2>/dev/null || echo '(unset)')"
echo "active: $(git config stacked-pr-gemini.mode 2>/dev/null || echo 'normal (default)')"
```

`git config stacked-pr-gemini.mode` resolves the local value first, then the global one.
If neither is set, the active mode is **normal**.

### 2. Ask the user what they want

If the user's invocation already made it clear (e.g. `/stacked-pr-gemini setup stacked`,
`/stacked-pr-gemini setup normal`, `/stacked-pr-gemini setup --global stacked`), skip the question and
...
```bash
git config --global stacked-pr-gemini.mode stacked   # or: normal
```

Local (this repo only):

```bash
git config --local stacked-pr-gemini.mode stacked    # or: normal
```

To clear a setting and fall back to the default / the other scope:

```bash
git config --global --unset stacked-pr-gemini.mode   # or --local
```
...
> stacked-pr-gemini.mode normal` in that repo.
...
```bash
git config stacked-pr-gemini.mode 2>/dev/null || echo 'normal (default)'
```
...
`/stacked-pr-gemini` bundles a AfterTool hook (see [nudge.md](nudge.md)) that nudges you
...
skill's `scripts/` directory (e.g. `~/.gemini/skills/stacked-pr-gemini/scripts`):
...
to run on every `/stacked-pr-gemini setup`. Re-run it if you ever see a "binary not found"
note in your hook logs.

## Important

- This subcommand reads and writes `git config stacked-pr-gemini.mode` and provisions the
  nudge hook binary (step 5). It never commits, pushes, or opens PRs.
- `stacked-pr-gemini.mode` is plain git config — the user can also set it by hand with
  `git config [--global] stacked-pr-gemini.mode <normal|stacked>`.
