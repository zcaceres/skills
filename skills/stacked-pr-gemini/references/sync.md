# `/stacked-pr-gemini sync` — Rebase the Stack onto Updated Trunk
...
> them first, then re-run `/stacked-pr-gemini sync`."
...
for i in "${!STACK[@]}"; do
  ...
  git rebase "$PARENT" || {
    echo "Conflict rebasing $BRANCH onto $PARENT — resolve and re-run /stacked-pr-gemini sync"
    break
  }
done
```
...
> `/stacked-pr-gemini sync` to push the rebased branches."
