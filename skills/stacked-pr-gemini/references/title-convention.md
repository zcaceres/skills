...
- [`/stacked-pr-gemini submit`](submit.md) — the publish point on the git-stack path.
...
- [`/stacked-pr-gemini update`](update.md) — on its stacked (git-stack) path, which
  re-publishes the stack.
- [`/stacked-pr-gemini checkpoint`](checkpoint.md) — **only** on its `gh`-fallback path,
  which still publishes eagerly. The git-stack `checkpoint` is local and
  publishes nothing, so it does not renumber.

It is **not** run by [`/stacked-pr-gemini merge`](merge.md). As PRs land bottom-up, the
...
- **Raw `git stack submit` (outside `/stacked-pr-gemini`)** resets titles to its own
  defaults. The next `/stacked-pr-gemini submit` or `/stacked-pr-gemini checkpoint` re-applies markers.
