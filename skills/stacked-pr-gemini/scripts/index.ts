...
const HOME = homedir();
const STATE_DIR = join(HOME, ".gemini", "state");
const STATE_FILE = join(STATE_DIR, "stacked-pr-gemini-nudge.json");
...
const THRESHOLD_LINES = Number(process.env.STACKED_PR_NUDGE_LINES ?? process.env.PR_NUDGE_LINES ?? 200);
const THRESHOLD_FILES = Number(process.env.STACKED_PR_NUDGE_FILES ?? process.env.PR_NUDGE_FILES ?? 4);
...
export function buildNudgeMessage(lines: number, files: number): string {
  return (
    ...
    `If this work forms a shippable slice, run /stacked-pr-gemini checkpoint to land it as a stacked PR before continuing.`
  );
}
...
async function main(): Promise<void> {
  try {
    ...
    emitNudge(stats.lines, stats.files, data.hook_event_name || "AfterTool");
  } catch {
