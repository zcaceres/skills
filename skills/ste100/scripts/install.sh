#!/usr/bin/env bash
# Wire the ste100 SessionStart hook into a Claude Code settings.json so the
# ASD-STE100 rules are injected at the start of every session (and after context
...
#   scripts/install.sh --no-statusline # wire the hooks only; skip the badge
#
# The status-line badge is added by default: install.sh saves any existing
# `.statusLine` and replaces it with the ste100 wrapper (statusline.sh), which
# runs your original status line and appends "◆ ste100" when the rules are on.
# uninstall.sh restores the saved original. Remove just the badge (keeping the
# rules) with: uninstall.sh --statusline-only.
#
...
# to keep the rules on across sessions. Toggle them with `/ste100 on|off`.

set -euo pipefail

SKILL_NAME="ste100"
# Two hooks: SessionStart injects the full rules; UserPromptSubmit restates them
...
if [ "$WIRE_STATUSLINE" -eq 1 ]; then
  ...
  # Detect our own managed wrapper by the env-var marker we always write, not by
  # "statusline.sh": another skill (laconic) ships a wrapper with the same
  # basename, and we want to chain it rather than mistake it for ours.
  if jq -e '(.statusLine.command // "") | contains("STE100_STATUSLINE_ORIG")' "$TARGET" > /dev/null 2>&1; then
    ...
    # A hand-added ste100 reference exists but isn't our wrapper. Wrapping it
    # would double the badge, so leave it and let the user decide.
    echo "⚠ $TARGET already has a hand-added 'ste100' reference in its statusLine."
    ...
    NEW_CMD="STE100_STATUSLINE_ORIG='$STATUSLINE_ORIG' '$STATUSLINE_WRAPPER'"
    backup_once
...
echo
echo "Now enable it: /ste100 on   (or run scripts/ste100.sh on)."
