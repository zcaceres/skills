...
# (saved by install.sh) with that same JSON, and appends the ste100 badge
# ("◆ ste100") when the rules resolve on for the workspace. When ste100 is off
# the badge is empty, so the line is exactly the original. The wrapper is
# invisible until you turn ste100 on.
#
# Env:
#   STE100_STATUSLINE_ORIG  path to the saved original `.statusLine` JSON that
...
orig_out=""
orig_file="${STE100_STATUSLINE_ORIG:-}"
...
badge="$(CLAUDE_PROJECT_DIR="$cwd" "$here/ste100.sh" statusline 2>/dev/null || true)"
